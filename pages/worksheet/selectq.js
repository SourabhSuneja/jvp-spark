...
// Function to fetch question data from server and return a promise
...
function generateIndexMap(questions, basis) {
   ...
   return {
      "map": indexMap,
      "uniqueValues": uniqueBasisValues
   };
}
...
function getQuestionIndicesByChapter(questions, chapterStartPoints, chapterNames) {
   ...
   for (let i = 0; i < chapterNames.length; i++) {
      ...
      chapters[chapterName] = Array.from({
         length: end - start
      }, (_, index) => start + index);
   }
...
function filterArray(array1, array2, minLen) {
   ...
   if (minLen != undefined && filteredArray.length < minLen) {
      ...
      if (qContainers['settings']['randomiseSelection']) {
         items = selectRandomItems(removedItems, diff);
      } else {
...
async function start(qbIds) {
   ...
   let {
      map: cardIndexMap,
      uniqueValues: uniqueCardIndices
   } = generateIndexMap(questions, "card");


   // generate index map on the basis of question type
   let {
      map: qTypeIndexMap,
      uniqueValues: uniqueQTypeIndices
   } = generateIndexMap(questions, "qType");

   // generate index map on the basis of media embedded in questions
   let {
      map: mediaEmbeddedIndexMap,
      uniqueValues: uniqueMediaEmbeddedIndices
   } = generateIndexMap(questions, "mediaEmbedded");
