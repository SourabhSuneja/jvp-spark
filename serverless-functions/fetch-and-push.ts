...
serve(async (req) => {
   ...
   try {
      ...
      const {
         ...
         error: subsError
      } = await getSubscriptions(supabase, target_type, target_tokens);
      ...
      // Step 5.1: Deduplicate subscriptions based on the endpoint
      const uniqueSubscriptionsMap = new Map();
      allSubscriptions.forEach(sub => {
         const endpoint = sub.subscription_object?.endpoint; // Get endpoint from JSON
         if (endpoint) {
            uniqueSubscriptionsMap.set(endpoint, sub);
         }
      });

      const subscriptions = Array.from(uniqueSubscriptionsMap.values()); // This is the new, unique list
